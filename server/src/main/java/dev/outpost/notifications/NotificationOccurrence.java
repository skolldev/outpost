...
/**
 * A trigger occurrence handed across the publisher seam: the one thing existing code tells the
 * notifications module about. Sealed so the formatter/delivery switch over its variants stays
 * exhaustive.
 */
public sealed interface NotificationOccurrence {
	...
	/**
	 * An Uptime Monitor's third consecutive failed check opened an Incident. Fired exactly when
	 * the incident row is inserted, not on every failed check or when an edit re-arms an
	 * already-open incident; monitors have no name column, so {@code monitorUrl} doubles as the
	 * identity and the payload's URL.
	 *
	...
	/**
	 * An Admin verifying a channel. Never stored in {@code notification_channel.triggers}, and
	 * bypasses channel matching (the target is named directly) — but still formatted and
	 * delivered through the same pipeline, so a successful test proves the whole path end to end.
	 *
